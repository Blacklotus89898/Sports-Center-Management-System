...
public class SpecificClassService {
    ...
    public SpecificClass createSpecificClass(int classId, String className, int year, String specificClassName, String description, LocalDate date, LocalTime startTime, int hourDuration, int maxCapacity, int currentCapacity, double registrationFee) {
        ...
        if (className == null) {
            ...
            throw new SCSException(HttpStatus.BAD_REQUEST, "Current capacity must be greater than 0.");
        ...
        if (classType == null){
            throw new SCSException(HttpStatus.NOT_FOUND, "Class type with name " + className + " not found.");
        }
    ...
    @Transactional
    public List<SpecificClass> getAllSpecificClasses(int classId) {
