    ...
    for (let i = 0; i < 1440; i += 30) {
      ...
      if (!isEventSlot) {
        ...
        timeSlots.push(
          <div
            key={`time-${i}`}
            className="time-slot w-16 text-gray-500 opacity-0 hover:opacity-100 transition-opacity duration-300 font-semibold"
    ...
    if (selectedEventIndex !== null) {
      ...
      return (
        <div className="event-popup fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-12 rounded-lg shadow-2xl z-50 w-11/12 sm:w-5/6 md:w-2/3 lg:w-1/2 xl:w-2/5">
          ...
          <p className="text-base-200 mb-6 text-xl">
            {formatTime(event.startTime)} - {formatTime(event.endTime)}
          </p>
